...
\title{Fisher's Exact Test: Drug Treatments and Disease Outcome}
\author{}
\date{}
...
Suppose there are three drug treatments (Drug A, Drug B, and Drug C) with the outcome of disease or no disease.

The goal is to test whether there is an association between drug treatments and disease outcomes using Fisher's Exact Test and post-hoc analysis.
...
\[
H_0:
\text{Drug treatment and disease outcome are independent}
\]
...
\[
H_1:
\text{Drug treatment and disease outcome are associated}
\]
...
\section*{Step 4: Overall Association Test}
...
\begin{itemize}
    \item Drug treatment
    \item Disease outcome
\end{itemize}

This means disease recovery differs among the drug treatments.

\section*{Step 5: Post-Hoc Fisher's Exact Tests}
...
\begin{itemize}
    \item Drug A performs significantly better than Drug B.
    \item Patients using Drug A are much more likely to recover.
\end{itemize}
...
\begin{itemize}
    \item Drug A performs significantly better than Drug C.
    \item Drug A leads to higher recovery rates.
\end{itemize}
...
\begin{itemize}
    \item Drug C performs significantly better than Drug B.
    \item Drug B has the worst disease outcome among the three treatments.
\end{itemize}

\section*{Step 6: Bonferroni Adjusted p-values}
...
\begin{itemize}
    \item All adjusted p-values remain below 0.05.
    \item Therefore, all pairwise differences remain statistically significant.
\end{itemize}
...
Therefore, treatment choice significantly impacts disease recovery.
...
\title{Zero-Inflated Poisson Regression on Fish Catch Data}
\author{}
\date{}
...
The coefficient for persons is positive and significant.
...
The coefficient for child is negative and significant.
...
The coefficient for camper is positive and significant.
...
The inflation model explains the probability of belonging to the always-zero group.
...
The Zero-Inflated Poisson regression model is suitable for this dataset because of excess zero counts. The analysis indicates that larger groups and campers tend to catch more fish, while the presence of children reduces fish catch.
...
\title{Zero-Truncated Poisson Regression on Hospital Length of Stay}
\author{}
\date{}
...
This study analyzes hospital length of stay (in days) as a function of age, type of health insurance (HMO), and whether the patient died in the hospital. Since hospital stay is recorded as at least one day, zero values are not possible. Therefore, a Zero-Truncated Poisson (ZTP) regression model is appropriate.
...
The coefficient of age is positive and significant.

\[
IRR = e^{0.0038} \approx 1.004
\]

This indicates that each additional year of age increases expected hospital stay by about 0.4\%.

\subsection{HMO Insurance}

The coefficient for HMO insurance is negative and significant.

\[
IRR = e^{-0.1472} \approx 0.863
\]

Patients with HMO insurance have approximately 13.7\% shorter hospital stays than non-HMO patients.

\subsection{Died in Hospital}

The coefficient for death is positive and highly significant.

\[
IRR = e^{0.5925} \approx 1.81
\]

Patients who died in the hospital have about 81\% longer stays than those who survived.

\section{Conclusion}

A Zero-Truncated Poisson regression model was used because hospital stay has no zero values. The results show that:
...
\title{Zero-Truncated Negative Binomial Regression on Hospital Length of Stay}
\author{}
\date{}
...
The Zero-Truncated Negative Binomial model is appropriate due to truncation at zero and overdispersion. Results show:
